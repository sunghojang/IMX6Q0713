 ...
 */
//by crazyboy 20151022 by treego
#ifndef DEBUG
	...
	while (!ts->stopped && tsc2007_is_pen_down(ts)) {
		...
		if (rt <= ts->max_rt) {
			dev_dbg(&ts->client->dev,
				...
				tc.x, tc.y, rt);
		//by crazyboy 20151104 by treego 	
			MANGO_DBG("DOWN point(%4d,%4d), pressure (%4u)\n",tc.x, tc.y, rt);
...
static int tsc2007_open(struct input_dev *input_dev)
{
	...
	enable_irq(ts->irq);
#ifndef CONFIG_MANGO_TSC2007_IRQ_DISABLE//crazyboys 20160524
			 ...
			 const struct i2c_device_id *id)
{
	...
	tsc2007_open(input_dev);//by crazyboy 20151110 by treego 
#endif
