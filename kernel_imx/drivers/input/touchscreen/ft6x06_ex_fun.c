...
static unsigned char CTPM_FW[] = {
	#include "SEWON_Medical_device_Ver0x04_app.i"
};
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	if (/*the firmware in touch panel maybe corrupted */
		...
		//MANGO_DBG("[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);
			  ...
			  u32 dw_lenth)
{
	...
	MANGO_DBG("[FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n", reg_val[0], reg_val[1]);
	...
	MANGO_DBG("Step 4:erase app and panel paramenter area\n");
	...
	bt_ecc = 0;
	MANGO_DBG("Step 5:write firmware(FW) to ctpm flash\n");
	...
	if ((dw_lenth) % FTS_PACKET_LENGTH > 0) {
		...
		for(i = 0;i < 30;i++)
		{
			...
			if(0xb0 == (reg_val[0] & 0xf0) && (0x03 + (j % 0x0ffd)) == (((reg_val[0] & 0x0f) << 8) |reg_val[1]))
			{
				MANGO_DBG("[FTS] write a block data finished \n");
				break;
			}
			msleep(1);
		}
	...
	MANGO_DBG("Step 6: read out checksum\n");
	...
	MANGO_DBG("Step 7: reset the new FW\n");
	auc_i2c_write_buf[0] = 0x07;
	ft6x06_i2c_Write(client, auc_i2c_write_buf, 1);
...
//by crazyboy 20151019 by treego 
#if 0
